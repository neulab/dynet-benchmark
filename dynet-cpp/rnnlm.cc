...
struct RNNLanguageModel {
  ...
  Expression calc_lm_loss(const vector<int> & sent, Dict & vw, ComputationGraph & cg) {
...
int main(int argc, char** argv) {
  ...
  for(int iter = 0; iter < 50; iter++) {
    ...
    for(auto & s : train) {
      ...
      if(i % 10000 == 0) {
        all_time += (system_clock::now() - start).count();
        int dev_words = 0;
        float dev_loss = 0;
        for(auto & sent : test) {
          ComputationGraph cg;
          Expression loss_exp = rnnlm.calc_lm_loss(sent, vw, cg);
          dev_loss += as_scalar(cg.forward(loss_exp));
          dev_words += sent.size();
        }
        cout << "nll=" << dev_loss/dev_words << ", ppl=" << exp(dev_loss/dev_words) << ", time=" << all_time << ", word_per_sec=" << all_tagged/all_time << endl;
        if(all_time > 300)
          exit(0);
      ...
      ComputationGraph cg;
      Expression loss_exp = rnnlm.calc_lm_loss(s, vw, cg);
