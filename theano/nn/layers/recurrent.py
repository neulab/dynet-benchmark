...
class LSTM(Layer):
    ...
    def get_mask(self, mask, X):
        """
        reshape mask to (time, nb_samples, 1)
        add broadcast to the last axis
        """
        if mask is None:
            mask = T.ones((X.shape[0], X.shape[1]))
...
class BiLSTM(Layer):
    ...
    def __call__(self, X, mask=None, init_state=None):
        ...
        hidden_states_forward = self.forward_lstm(X, mask, init_state)
        hidden_states_backward = self.backward_lstm(X[:, ::-1, :], mask[:, ::-1], init_state)
