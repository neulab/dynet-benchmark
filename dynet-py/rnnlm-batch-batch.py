...
def batch(cols):
    total,nrows,ncols,nbatch=cols[0].dim()
    assert(nbatch==1)# doesn't currently work with batched
    cols_ = dy.concatenate_cols(cols)
    return dy.reshape(cols_, (nrows,ncols), batch_size=len(cols))

def unbatch(B):
    d = B.dim()
    return split_cols(dy.reshape(B,(d[1],d[3]), batch_size=1))
...
def calc_lm_loss(sents):
  ...
  sequence = [init_ids]
  sequence.extend(wids[:-1]) # no need to enter the last element, which is EOS or equiv.
  outputs = f_init.transduce([dy.lookup_batch(WORDS_LOOKUP, x) for x in sequence])
  if BATCHED_SENT:
    scores = unbatch(W_exp*batch(outputs)+b_exp)
  else:
    scores  = [b_exp+(W_exp * o) for o in outputs]
  assert(len(scores)==len(wids))
  losses = [dy.pickneglogsoftmax_batch(score, wid) for (score,wid) in zip(scores,wids)]

  for i,mask in enumerate(masks):
    if mask[-1] != 1:
        mask_expr = dy.inputVector(mask)
        mask_expr = dy.reshape(mask_expr, (1,), len(sents))
        losses[i] = losses[i] * mask_expr
